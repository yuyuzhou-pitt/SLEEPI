...
SYSCALL=futex
RUNPLOT=y
HISTOW=1 # statistic width in us. Set 0 if not using, or it overrides -n/--number.
...
#then
    ...
    for data in `ls -d1 ${DATABAK}/*.data`; do
        ...
        python MultiThread_SyscallPlot.py -w ${HISTOW} -n ${HISTON} -s ${SYSCALL} -i ${data} -o ${data}.out > /dev/null 2>&1
