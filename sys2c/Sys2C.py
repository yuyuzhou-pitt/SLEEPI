...
def l2c_addr_201(mmapaddr, func):
    return re.sub("\(0x[0-9a-f]*\,", "(%s," % mmapaddr, func)   
...
def line2c(sysname, func, retvalue, retdata):
    ...
    if "open" == sysname or "openat" == sysname:
        ...
        if re.search('NULL', func):
            ...
            if not mmapaddr:
                mmapaddr = "addr_%s" % timestr
                func = l2c_addr_201("NULL", func)
                retdata["mmapaddr"] = mmapaddr
                funcline = "void *%s; %s = %s;\n" % (mmapaddr, mmapaddr, func)
            else:
                # no new variable defined if alread exists
                #print func
                funcline = "%s;\n" % l2c_addr_201(mmapaddr, func)
        ...
        funcline = "%s;\n" % l2c_100(func)
    elif "210" == sysDict[sysname]: 
        if munmapaddr:
            funcline = "%s;\n" % l2c_addr_201(munmapaddr, func)
            retdata.pop("munmapaddr", None) # remove after unmmap
        elif mmapaddr:
            funcline = "%s;\n" % l2c_addr_201(mmapaddr, func)
        else:
            funcline = "%s;\n" % l2c_addr_201("addr", func)
